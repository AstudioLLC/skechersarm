<div>
    ...
    <div class="row pb-3">
        <div class="col-6">
            <a class="btn btn-light py-3 px-5 text-black w-100" href="{{route('admin.slide.create')}}">Create new {{$this->name}}</a>
        </div>
        <div class="col-6">
            <a href="{{route('admin.slides.trashed')}}" class="float-right btn btn-light py-3 px-5 text-black w-100">Trashed</a>
                    ...
                    <table class="table table-striped">
                        ...
                        @foreach($items as $item)
                            <tr  wire:sortable.item="{{$item->id}}" wire:key="slide-{{$item->id}}" >
                                ...
                                <td>
                                    <img src="{{asset('images/slides')}}/{{$item->image}}" class="avatar avatar-sm me-3" alt="user1">
                                </td>
                                <td>
                                    {{$item->title}}
                                </td>
                                ...
                                <td>
                                    <a href="{{route('admin.slide.edit',['id'=>$item->id ?? null])}}" class="px-2">
