<div>
    <div class="row pb-3">
        <div class="col-3">
            <a class="btn btn-light py-3 px-5 text-black w-100" href="{{route('admin.poll.create')}}">Create new {{$this->name}}</a>
        </div>
        <div class="col-3">
            <a href="{{route('admin.trashed',['model' => 'Poll','path' => 'polls'])}}" class="btn btn-light py-3 px-5 text-black w-100">
                Trashed {{$name}}
            </a>
        </div>
                        ...
                        <table class="table table-striped">
                            ...
                            @foreach($items as $item)
                                <tr  wire:sortable.item="{{$item->id}}" wire:key="item-{{$item->id}}" >
                                    ...
                                    <td>
                                        <a href="{{route('admin.poll.options',['id' => $item->id])}}">
                                            <i class="fas fa-poll"></i>
                                        </a>
                                        <a href="{{route('admin.poll.edit',['id' => $item->id])}}" >
