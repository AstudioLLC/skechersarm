...
<div>
    @if($errors->any())
        <h1>{{$errors}}</h1>
    @endif
    <form class="form-horizontal" enctype="multipart/form-data" wire:submit.prevent="updatePage">
        <div class="row">
            <div class="col-md-7">
                <div class="form-group has-danger" wire:ignore>
                    ...
                    <third-tab-container >
                        ...
                        <third-tab-content>
                            <input type="text" name="title.en"  wire:keyup="generateSlug"  class="form-control  @error('title.en') form-control-alternative is-invalid @enderror title-en" id="en2" wire:model.defer="title.en" placeholder="Enter English Title">
                        </third-tab-content>
                        <third-tab-content>
        ...
        <div class="form-group has-danger">
            <livewire:admin.supports.form-title name="Content"/>
            <tab-container wire:ignore>
                ...
                <tab-content>
                    <textarea type="text" name="content.en" class="form-control content-en" id="content_en" wire:model.defer="content.en" placeholder="Enter English Description"></textarea>
                </tab-content>
                <tab-content>
                    <textarea type="text" name="content.hy" class="form-control content-hy" id="content_hy" wire:model.defer="content.hy" placeholder="Enter Armenian Description"></textarea>
                </tab-content>
                <tab-content>
                    <textarea type="text" name="content.ru" class="form-control content-ru" id="content_ru" wire:model.defer="content.ru" placeholder="Enter Russian Description"></textarea>
                </tab-content>
        ...
        <div class="row" >
            <div class="col-md-3">
                <div class="form-group ">
                    <livewire:admin.supports.form-title name="To Top"/>
                    <div class="form-check form-switch active-toggle" wire:ignore>
    {{--                    <input class="form-check-input " wire:model.lazy="to_top" type="checkbox" role="switch" checked value="">--}}
                            @livewire('admin.supports.toggle-switch',  ['model' => $item, 'field' => 'to_top'], key($item->id))
                    </div>
                </div>
...
{{--            @dd($selectedPage)--}}
           @if($selectedPage->static == null)
                <div class="col-md-3">
                    <div class="form-group">
                        <label for="selectedParent" class="col-md124 col-form-label text-md-right">Parent Page</label>
                        <select wire:model.lazy="selectedParent" class="form-control">
                            <option value="{{null}}" selected>Choose Parent Page</option>
                            @if(count($parents))
                            @foreach($parents as $parent)
                                   <option value="{{ $parent->id }}" @if($selectedPage->parent_id) selected @endif>{{ $parent->parentPages()->exists() ? '-- ' .'('  .$parent->parentPages->title. ') '. $parent->title : $parent->title  }}</option>
                            @endforeach
                            @else
                                @foreach($dinamic_page as $parent)
        ...
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="form-group ">
                    ...
                    @elseif($image)
                        <img src="{{asset('images/pages')}}/{{$image}}" class="img d-block mb-2 w-100 rounded uploading-image">
                    @endif
...
@push('js')
    <script>
        ...
        CKEDITOR.instances.content_en.on('change', function() {
        @this.set('content.en', this.getData());
        });
        CKEDITOR.replace('content_hy');
        CKEDITOR.instances.content_hy.on('change', function() {
        @this.set('content.hy', this.getData());
        });
        CKEDITOR.replace('content_ru');
        CKEDITOR.instances.content_ru.on('change', function() {
        @this.set('content.ru', this.getData());
        });
    </script>
@endpush
