<div>
    <div class="row pb-3">
        <div class="col-6">
            <a class="btn btn-light py-3 px-5 text-black w-100" href="{{route('admin.delivery-region.create')}}">Create new {{$this->name}}</a>
        </div>
    </div>
...
{{--                                            @if(count($forSellerPage->childpages))--}}
                                                <a class="text-xs text-secondary mb-0"
                                                   href="{{ route('admin.delivery-cities', ['id' => $item->id]) }}">
                                                    Go to settlements ({{count($item->cities)}})
                                                </a>
{{--                                            @endif--}}
                                        </div>
                                    </td>
                                    <td class="align-middle">
                                        <a href="{{route('admin.delivery-region.edit',['url'=>$item->id ?? null])}}" class="px-2">
