                ...
                <div class="accordion" id="accordionPanelsStayOpenExample">
                    ...
                    <div class="accordion-item shadow-sm mb-4 border-0">
                        <h2 class="accordion-header" id="panelsStayOpen-headingOne">
                            <button class="accordion-button justify-content-between flex-wrap" type="button" data-bs-toggle="collapse" data-bs-target="#orderId{{$key}}" aria-expanded="true" aria-controls="orderId{{$key}}">
                                <div class="acc-title">
                                    <p class="mb-1">{{__('frontend.ongoing.Order No')}} {{$item->order_id}}</p>
                                    <p class="mb-0">{{$item->created_at}}</p>
                                </div>
                                <div class="acc-status">
                                    <p class="mb-1">{{__('frontend.ongoing.Order Status')}}  -  @if($item->status =='finished'){{__('frontend.ongoing.Completed')}}@endif
                                                                            @if($item->status =='canceled'){{__('frontend.ongoing.Rejected')}}@endif
                                    </p>
                                ...
                                <div class="shopping-cart">
                                    ...
                                    <div class="shopping-cart-info">
                                        ...
                                        <div class="shipping-cart-item shop-col-img">
                                            <img src="{{asset('images/products')}}/{{$orderItem->product->image}}" alt="" class="shopping-cart-item-img">
                                        </div>
                                        @endif
                                        ...
                                        <div class="shipping-cart-item shop-col-price">
                                            ...
                                            <div class="d-flex flex-row flex-sm-column align-items-center">
                                                ...
                                                <span class="badge bg-danger rounded-0 fs-7">-{{$orderItem->product->sale_percent}}%</span>
                                                @endif
                                                @if(isset($orderItem->product->sale_price))
                                                <p class="text-muted badge d-block text-decoration-line-through m-0">{{($orderItem->product->sale_price)?$orderItem->product->price :null}} ֏</p>
                                                    @endif
                                                    @if(isset($orderItem->product->sale_price))
                                                <p class="m-0">{{($orderItem->product->sale_price != null) ? $orderItem->product->sale_price :$orderItem->product->price}} ֏</p>
                                                    @endif
                                            </div>
                                        </div>
